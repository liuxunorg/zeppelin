...
if [[ "${INTERPRETER_ID}" == "spark" ]]; then
  ...
  export SUBMARINE_DOCKER_JAVA_INTP_OPTS="${DOCKER_JAVA_HOME}/bin/java -Dfile.encoding=UTF-8 -Dlog4j.configuration=file:///zeppelin/conf/log4j.properties "
  ...
  if [[ -n "${HADOOP_YARN_SUBMARINE_JAR}" ]]; then
    if [[ -f "${HADOOP_YARN_SUBMARINE_JAR}" ]]; then
      export SUBMARINE_RUNNER="${YARN_BIN} jar ${HADOOP_YARN_SUBMARINE_JAR} "
      ...
      SUBMARINE_RUNNER="${SUBMARINE_RUNNER} --env TZ=\"${DOCKER_CONTAINER_TIME_ZONE}\" "
      SUBMARINE_RUNNER="${SUBMARINE_RUNNER} --env HADOOP_LOG_DIR=/tmp "
      ...
      SUBMARINE_RUNNER="${SUBMARINE_RUNNER} --docker_image ${SUBMARINE_INTERPRETER_DOCKER_IMAGE} "
      SUBMARINE_RUNNER="${SUBMARINE_RUNNER} --input_path hdfs:/// "
      SUBMARINE_RUNNER="${SUBMARINE_RUNNER} --worker_resources memory=8G,vcores=1,gpu=0 "
      SUBMARINE_RUNNER="${SUBMARINE_RUNNER} --num_workers 1 "
      SUBMARINE_RUNNER="${SUBMARINE_RUNNER} --keytab ${SUBMARINE_HADOOP_KEYTAB} "
      SUBMARINE_RUNNER="${SUBMARINE_RUNNER} --principal ${SUBMARINE_HADOOP_PRINCIPAL} "
      SUBMARINE_RUNNER="${SUBMARINE_RUNNER} --distribute_keytab "
    fi
  fi
fi
...
if [[ -n "${SPARK_SUBMIT}" ]]; then
    ...
    if [[ "${INTERPRETER_LAUNCH_MODE}" == "yarn" ]]; then
        INTERPRETER_RUN_COMMAND+=' '` echo ${SUBMARINE_RUNNER} --worker_launch_cmd \"${SUBMARINE_DOCKER_JAVA_INTP_OPTS} ${ZEPPELIN_INTP_MEM} -cp ${SUBMARINE_INTP_CLASSPATH} ${ZEPPELIN_SERVER} ${CALLBACK_HOST} ${PORT} ${INTP_GROUP_ID} ${INTP_PORT}\"`
    else
